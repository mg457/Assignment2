...
public class DecisionTreeClassifier implements Classifier {
	...
	public void train(DataSet data) {
        ...
        if(depth == -1){
        	DecisionTreeNode root = trainRec(examples, -1, usedFeatures, splitDirection);
        	classifier = root;
            //System.out.println(root.treeString());

        }else if (depth ==0){
        	DecisionTreeNode root = new DecisionTreeNode(getPrediction(examples, usedFeatures, splitDirection));
            //System.out.println(root.treeString());
	...
	private DecisionTreeNode trainRec( ArrayList<Example> examples, int depth, ArrayList<Integer> usedFeatures, ArrayList<Double> splitDirection){
		...
		if(isInSameClass(examples, usedFeatures, splitDirection) || haveSameFeatureValues(examples, usedFeatures, splitDirection)){
        ...
        //System.out.println(depth + " " + trainingErrors );
        ...
        ArrayList<Double> splitLeft = (ArrayList<Double>) splitDirection.clone();
        splitLeft.add(0.0);
        ArrayList<Double>  splitRight = (ArrayList<Double>) splitDirection.clone();
        splitRight.add(1.0);
	...
	private double calculateTrainingError(int featureIndex, ArrayList<Example> examples, ArrayList<Integer> usedFeatures, ArrayList<Double> usedVals) {
		...
		//System.out.println(bin00 +" "+ bin01 +" "+ bin10 +" "+bin11+" "+count);
	...
	private boolean noDataInSubset(ArrayList<Example> examples, ArrayList<Integer> usedFeatures, ArrayList<Double> usedVals){
		...
		//System.out.println("no data in subset");
		return true;
	}
...
//		if(retVal == true){
//			System.out.println("have same feature vals");
//		}
		return retVal;
	}
